  ...
  try {
    ...
    cart.products.forEach(item => {
      ...
      div.innerHTML = `
        ...
        <div class="cart-item-price" style="margin-right:37px">${formatPrice(item.price * item.quantity)}</div>
...
document.addEventListener("click", async (e) => {
  ...
  if (removeId) {
    try {
      ...
      if (res.ok) loadCart();
      else console.error("Failed to remove item");
    } catch (err) {
      console.error("Remove error:", err);
    }
  }
...
// Clear authenticated user's cart (best-effort)
async function clearServerCart() {
  try {
    let res = await fetch(`${API_BASE}/api/cart/clear`, { method: "POST", credentials: "include" });
    if (res.ok) return;

    res = await fetch(`${API_BASE}/api/cart`, { method: "DELETE", credentials: "include" });
    if (res.ok) return;
    ...
    await Promise.all(cart.products.map(it =>
      fetch(`${API_BASE}/api/cart/${it.productId}`, { method: "DELETE", credentials: "include" }).catch(() => { })
    ));
  } catch (_) { }
}
...
// Function to add item to guest cart (to ensure price is included)
